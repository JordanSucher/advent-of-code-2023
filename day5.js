...
const getMinimumLocation = () => {
    ...
    allMapGroups.forEach(mappings => {
        ...
        mapInput.forEach(input=>{
            let relMapping = mappings.filter(mapping=>
                (input > mapping[1]) && (input < (mapping[1]+mapping[2]))    
            )
...
const getMinimumLocation2 = () => {
    ...
    mapInput.forEach((inputPair,index)=>{
            ...
            allMapGroups.forEach(mappings => {
                ...
                batchSeedArray.forEach(seed=>{
                    ...
                    if(mappingCache 
                        ...
                        relMapping = mappings.filter(mapping=>
                            (seed >= mapping[1]) && (seed < (mapping[1]+mapping[2])))
